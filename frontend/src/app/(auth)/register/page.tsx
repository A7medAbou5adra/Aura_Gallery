    ...
    <div className="min-h-screen flex flex-col bg-brand-dark">
      ...
      <div className="flex-1 flex items-center justify-center p-6 py-12">
        <div className="bg-brand-charcoal p-10 rounded-lg shadow-2xl border border-gray-800 w-full max-w-md relative overflow-hidden">
          ...
          <h2 className="text-3xl font-serif text-white mb-8 text-center">Join The Exclusive</h2>
          
          {error && <div className="bg-red-500/10 border border-red-500 text-red-500 p-3 rounded mb-6 text-sm">{error}</div>}
          
          <form onSubmit={handleRegister} className="space-y-6">
            <div>
              <label className="block text-gray-400 text-sm mb-2 uppercase tracking-wide">Full Name</label>
              <input 
                type="text" 
          ...
          <div className="mt-8 text-center text-gray-400 text-sm">
            Already have an account? <a href="/login" className="text-brand-gold hover:underline">Sign in</a>
          </div>
        </div>
      </div>
    </div>
